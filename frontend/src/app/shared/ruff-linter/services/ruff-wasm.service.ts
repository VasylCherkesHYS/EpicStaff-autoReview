...
export class RuffWasmService {
    private workspace: { check(contents: string): RuffDiagnostic[] } | null = null;
    ...
    async init(): Promise<void> {
        ...
        this.initPromise = (async () => {
            try {
                ...
                const wasmUrl = new URL('assets/ruff-wasm/ruff_wasm_bg.wasm', document.baseURI).href;
                await ruff.default(wasmUrl);
                this.workspace = new ruff.Workspace(RUFF_DEFAULT_CONFIG as object, ruff.PositionEncoding.Utf16);
            } catch (err) {
                throw err;
            }
        })();
