...
import { RealtimeAgent, UpdateRealtimeAgentRequest } from '../models/realtime-agent.model';
...
export class RealtimeAgentService {
    ...
    updateRealtimeAgent(agentId: string, agentData: UpdateRealtimeAgentRequest): Observable<RealtimeAgent> {
        const url: string = `${this.apiUrl}${agentId}/`;
        return this.http.put<RealtimeAgent>(url, agentData);
    }

    createRealtimeAgent(agentData: UpdateRealtimeAgentRequest): Observable<RealtimeAgent> {
