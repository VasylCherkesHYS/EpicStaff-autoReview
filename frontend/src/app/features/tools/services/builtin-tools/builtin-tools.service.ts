...
export class BuiltinToolsService {
    ...
    getTools(): Observable<Tool[]> {
        ...
        return this.http.get<ApiGetRequest<Tool>>(this.apiUrl, { params }).pipe(map((response) => response.results));
    }

    getToolsByIds(toolIds: number[]): Observable<Tool[]> {
        const requests: Observable<Tool>[] = toolIds.map((id) => this.http.get<Tool>(`${this.apiUrl}${id}/`));
