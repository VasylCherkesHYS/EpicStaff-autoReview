...
import { CreateCrewTagRequest, GetCrewTagRequest, UpdateCrewTagRequest } from '../models/crew-tag.model';
...
export class ProjectTagsApiService {
    ...
    getCrewTags(): Observable<GetCrewTagRequest[]> {
        return this.http.get<ApiGetRequest<GetCrewTagRequest>>(this.apiUrl).pipe(map((response) => response.results));
    }
    ...
    // Update crew tag
    updateCrewTag(id: number, tag: UpdateCrewTagRequest): Observable<GetCrewTagRequest> {
    ...
    patchUpdateCrewTag(id: number, updateData: Partial<UpdateCrewTagRequest>): Observable<GetCrewTagRequest> {
        return this.http.patch<GetCrewTagRequest>(`${this.apiUrl}${id}/`, updateData, {
            headers: this.httpHeaders,
        });
    }
