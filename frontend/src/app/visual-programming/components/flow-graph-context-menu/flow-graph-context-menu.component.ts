...
export type MenuType = 'flow-core' | 'project-core' | 'projects' | 'flows' | 'llms' | 'tools' | 'staff';
...
export class FlowGraphContextMenuComponent implements AfterViewInit, OnDestroy {
    ...
    public get topPosition(): number {
        return this.isMenuPositioned ? this.topValue : FlowGraphContextMenuComponent.OFFSCREEN_COORD;
    }

    public get leftPosition(): number {
        return this.isMenuPositioned ? this.leftValue : FlowGraphContextMenuComponent.OFFSCREEN_COORD;
    }
    ...
    public selectedMenu: MenuType = this.menuContext === 'flow-graph' ? 'flow-core' : 'project-core';
    ...
    public get menuWidth(): string {
        if (this.selectedMenu === 'flow-core' || this.selectedMenu === 'project-core') {
            return 'auto';
        }
        return '380px';
    }
    ...
    private schedulePositionUpdate(): void {
        ...
        this.positionUpdateTimeoutId =
            typeof window !== 'undefined' ? window.setTimeout(timeoutFn) : (setTimeout(timeoutFn) as unknown as number);
    }
    ...
    private updatePositionWithinViewport(): void {
        const desiredTop = this.menuContext === 'flow-graph' ? this.position.y - 80 : this.position.y - 130;
        ...
        const { width: menuWidth, height: menuHeight } = this.getMenuDimensions();

        const maxLeft = viewportWidth - menuWidth - FlowGraphContextMenuComponent.VIEWPORT_MARGIN;
        const maxTop = viewportHeight - menuHeight - FlowGraphContextMenuComponent.VIEWPORT_BOTTOM_MARGIN;

        this.leftValue = this.clamp(
            desiredLeft,
            FlowGraphContextMenuComponent.VIEWPORT_MARGIN,
            Math.max(FlowGraphContextMenuComponent.VIEWPORT_MARGIN, maxLeft)
        );
        this.topValue = this.clamp(
            desiredTop,
            FlowGraphContextMenuComponent.VIEWPORT_MARGIN,
            Math.max(FlowGraphContextMenuComponent.VIEWPORT_MARGIN, maxTop)
        );
