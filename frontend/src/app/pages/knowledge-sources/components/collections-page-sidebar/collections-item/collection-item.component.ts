        ...
        >
            <div
                ...
                [class.status-warning]="
                    collection.status === CollectionStatus.WARNING
                "
            ></div>
            ...
            this._embeddingConfigsService
                ...
                .subscribe({
                    next: (embeddingConfig) => {
                        this._pageService.setSelectedEmbeddingConfig(
                            embeddingConfig
                        );
                    },
                    error: (error) => {
            ...
            .subscribe((newName) => {
                if (newName) {
                    this._sourceCollectionService
                        ...
                        .subscribe({
                            next: () => {
                                this._pageService.updateCollection(
                                    collection.collection_id,
                                    {
        ...
        this._confirmationDialogService
            ...
            .subscribe({
                next: (confirmed) => {
                    ...
                    if (confirmed === true) {
                        this._sourceCollectionService
                            .deleteGetSourceCollectionRequest(
                                collection.collection_id
                            )
                            .pipe(takeUntil(this._destroy$))
                            .subscribe({
                                next: () => {
                                    this._pageService.removeCollection(
                                        collection.collection_id
                                    );

                                    if (
                                        this._pageService.selectedCollection()
                                            ?.collection_id ===
                                        collection.collection_id
                                    ) {
                                        ...
                                        if (remainingCollections.length > 0) {
                                            ...
                                            this._pageService.setSelectedCollection(
                                                null
                                            );
                                        }
                                    }
