...
def test_validate_missing_required_field_raises_error(validator, mock_tool):
    ...
    # Execute & Assert
    with pytest.raises(PythonCodeToolConfigSerializerError, match=f"Field '{field_name}' is required"):
...
@pytest.mark.parametrize("data_type, input_val, expected_val", [
    ...
    (PythonCodeToolConfigField.FieldType.LLM_CONFIG, "5", 5), # Mapped to int in code
])
@pytest.mark.django_db
