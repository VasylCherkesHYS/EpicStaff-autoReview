...
Generate a response of the target length and format that responds to the user's question, summarizing all information in the input reports appropriate for the response length and format while being as specific, accurate and concise as possible.
...
Base your response only on the information found in the provided reports. Do not use general knowledge or training data. If the reports do not contain information that answers the question, say that the provided documents do not cover it.
...
2. intermediate_answer: An answer to the query based only on the information found in the community summaries below. Do not use general knowledge or training data, and do not introduce facts that are not supported by the summaries. If the summaries do not contain information that answers the query, say so. This must be formatted in markdown and must begin with a header that explains how the following text is related to the query.
...
Use this information to help you decide whether or not you need more information about the entities mentioned in the report.
