...
class NgrokTunnel(AbstractTunnelProvider):
    ...
    async def _establish_connection(self):
        ...
        """
        print(f"Attempting to connect ngrok tunnel on port {self._port} (Region: {self._region or 'us'})...")
        
        # Use instance-specific config to avoid global state conflicts
        config = PyngrokConfig(
            auth_token=self._auth_token, 
            region=self._region if self._region else "us"
        )
        ...
        while self._is_running:
            ...
            if self._is_running and self._tunnel is None:
                try:
                    ...
                    logger.warning(f"Reconnection attempt failed: {e}. Next try in {self._reconnect_timeout}s")
    ...
    async def disconnect(self):
        ...
        tunnel_url = self._tunnel.public_url
        ...
        logger.info(f"Ngrok tunnel {tunnel_url} closed successfully.")