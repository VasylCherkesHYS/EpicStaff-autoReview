...
async def main():
    ...
    if settings.USE_TUNNEL:
        logger.info(
            f"Tunnel enabled. Attempting to use provider: '{settings.WEBHOOK_TUNNEL}'"
        )
        try:
            ...
            logger.error(
                f"Error: Configuration error for provider '{settings.WEBHOOK_TUNNEL}'. {e}",
                file=sys.stderr,
            )
            sys.exit(1)

    webhook_service = WebhookService(
        port=settings.WEBHOOK_PORT, tunnel_provider=tunnel_provider
    )
