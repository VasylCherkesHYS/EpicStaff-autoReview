...
class MCPToolNode:
    ...
    def __init__(self, cfg: MCPNodeConfig):
        if not cfg.services:
            raise ValueError("MCPNodeConfig.services is empty. Add at least one MCPService.")
        if cfg.default_service not in cfg.services:
            raise ValueError(f"default_service '{cfg.default_service}' not found in services.")
        self.cfg = cfg

    def _validate(self, service: str, tool_name: str) -> None:
        if service not in self.cfg.services:
            raise ValueError(f"Unknown service '{service}'. Known: {list(self.cfg.services.keys())}")
        allowed = self.cfg.allowed_tools.get(service, [])
        if allowed and tool_name not in allowed:
            raise ValueError(f"Tool '{tool_name}' is not allowed for service '{service}'. Allowed: {allowed}")

    async def _acall(self, service: str, tool_name: str, payload: Dict[str, Any]) -> Any:
    ...
    def call(self, tool_name: str, payload: Optional[Dict[str, Any]] = None, service: Optional[str] = None) -> Any:
        payload = payload or {}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--url", default="http://127.0.0.1:8080/mcp", help="MCP server URL")
    parser.add_argument("--tool", required=True, help="Tool name to call (e.g. run_browser)")
