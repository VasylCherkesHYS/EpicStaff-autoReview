...
.PHONY: backup apply-backup stash-tags apply-tags switch start-prod docker-generate-certs dev dev-down dev-logs rebuild-dev prod prod-down prod-logs clean help
...
dev:
	...
	@cd src && docker compose -f docker-compose.dev.yaml --env-file ./.env --env-file ../dev/dev.env up --build -d
